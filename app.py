...
st.caption(f"③ S2 콘텐츠 전체 (file3) 은 **{FILE3_PATH.relative_to(pathlib.Path(__file__).parent)}** 를 자동으로 사용합니다.")  # ★ 변경
...
if st.button("🟢 매핑 실행"):
    ...
    if not FILE3_PATH.exists():                                      # ★ 변경
        st.error(f"⚠️ 3번 파일이 {FILE3_PATH} 에 없습니다. 먼저 data 폴더에 all_contents.xlsx 를 넣어 주세요.")
        st.stop()
    ...
    # … (나머지 7) ~ 13) 구간은 변경 없이 그대로 두시면 됩니다) …
    ...
    pairs_unique = base_pairs.loc[~dup_mask].sort_values("매핑콘텐츠명").reset_index(drop=True)
    pairs_same   = base_pairs.loc[dup_mask].sort_values("매핑콘텐츠명").reset_index(drop=True)
    ...
    final_unmatch = (df2.loc[mask_pair, "정제_상품명"]
                       .drop_duplicates()
    ...
    st.download_button(
        "📥 결과 엑셀 다운로드",
        buf.getvalue(),
        file_name=save_name,   # ⬅️ 사용자가 지정한 파일명
