...
def clean_title(txt: str) -> str:
    ...
    t = re.sub(r"특별$", "", t)
    t = re.sub(r"\[[^\]]*\]", "", t)
    t = re.sub(r"[$begin:math:display$$end:math:display$]", "", t)   # 대괄호 남은 것 제거
...
# ➕ 저장 파일명 입력칸 (확장자 생략 가능) ────────────────────────────
file_name_input = st.text_input(
    "💾 저장할 파일 이름 (.xlsx 생략 가능)",
...
if st.button("🟢 매핑 실행"):
    ...
    df2 = pd.concat(pd.read_excel(file2, sheet_name=None).values(),
                    ignore_index=True)
    ...
    # ❼ 매핑콘텐츠명 / 콘텐츠ID 열 -----------------------------------
    mask_pair = df2["정제_상품명"] == df2["매핑결과"]   # 둘이 같을 때!
    pairs = (
    ...
    titles_used   = set(pairs["매핑콘텐츠명"])
    final_unmatch = no1[~no1.isin(map3.index) & ~no1.isin(titles_used)] \
                      .drop_duplicates()
    ...
    # 👉 저장 파일명 최종 확정
    save_name = file_name_input.strip()
