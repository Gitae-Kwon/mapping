...
import openpyxl, xlsxwriter        # requirements.txt 에 포함
...
mask_pair = df2["정제_상품명"] == df2["매핑결과"]          # “둘이 같을 때”만 추출
pairs = (
    df2.loc[mask_pair, ["정제_상품명", "최종_매핑결과"]]   # ★ '최종_매핑결과' 사용
       .drop_duplicates()
       .sort_values("정제_상품명")
...
pad = len(df2) - len(pairs)                  # ← 같은 들여쓰기(0-칼럼)
...
df2["콘텐츠ID"]     = list(pairs["콘텐츠ID"])     + [""] * pad
    ...
    #   b. ‘최종_매핑결과’ 다음에 새 두 열 배치
    cols = list(result.columns)
    i = cols.index("최종_매핑결과") + 1
    cols.remove("매핑콘텐츠명"); cols.remove("콘텐츠ID")
    cols[i:i] = ["매핑콘텐츠명", "콘텐츠ID"]
    #   c. 맨 앞 + 나머지
